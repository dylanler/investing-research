...
async function loadReportData(): Promise<ReportData> {
  ...
  const stockRecommendations = parseCsvObjects(stocksCsv).map((row) => ({
    ...
    sourceTags: parseSourceTags(row.source_tags),
    alphaScore: toOptionalNumber(row.alpha_score),
    alphaRank: toOptionalNumber(row.alpha_rank),
    alphaNote: row.alpha_note,
  ...
  sources.push({
    ...
    usedFor: 'June 23, 2026 latest prices and YTD return refresh for public stock-exposure proxies.',
  });
