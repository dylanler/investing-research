#!/usr/bin/env python3
"""Apply June 9, 2026 agent verdicts into the established refresh/overlay pipeline.
...
VERDICTS = ROOT / "tmp" / "agent-2026-06-09" / "verdicts"
SUMMARY = ROOT / "tmp" / "agent-2026-06-09" / "summary.json"
CTX_SYMBOLS = ROOT / "tmp" / "agent-context-2026-06-09" / "symbols"
TODAY = "2026-06-09"
TODAY_LABEL = "June 9, 2026"
...
REMOVALS = {
    "latent-ai-nodes": {"FARO"},
    "latent-ai-nodes-strict": {"GRC", "GTLS"},
    "scaling": {"POET"},
    "companies": {"WOLF"},
    "ai-passives-alpha": {"COHR"},
}
...
def apply_latent_broad():
    ...
    for c in [c for c in accepted if c.get("targetPage") == "latent-ai-nodes"]:
        ...
        key = "AGENT_20260609_" + case_key(c.get("yahooSymbol") or c["ticker"])[:12]
        rows.append({
...
def bump_labels():
    ...
    for rel in targets:
        p = ROOT / rel
        t = p.read_text()
        n = t.replace("June 8, 2026", TODAY_LABEL)
        if n != t:
...
def main():
    ...
    bump_labels()
    if not DRY:
        (ROOT / "tmp" / "agent-2026-06-09" / "apply_log.txt").write_text("\n".join(changes_log) + "\n")
